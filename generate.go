...
type Generator struct {
	...
	PrefilterCallback func(fname string, fpath string, fcontent []byte) []byte
}
...
func (x *Generator) Write(wr io.Writer) error {
	...
	fmt.Fprintln(writer)
	fmt.Fprintln(writer, "\t\"github.com/tarokker/go-assets\"")
	fmt.Fprintln(writer, ")")
	fmt.Fprintln(writer)
	...
	if x.fsFilesMap != nil {
		...
		for k, v := range x.fsFilesMap {
			...
			if x.PrefilterCallback != nil {
				data = x.PrefilterCallback(v.info.Name(), v.path, data)
			}
